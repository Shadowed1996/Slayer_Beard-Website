...
| `programmaSostituito(orari, adessoMs)` | `{ evento, giorni }`: l'evento acceso e, giorno per giorno (0–6), se la sua diretta regolare **finisce sotto** all'evento. Sfiorarsi non conta: la serata che comincia quando la maratona finisce si fa davvero. Un evento **senza durata** non si prende nessun giorno: la sua fine non si sa, e l'anno finto (`ORE_APERTO`) cancellerebbe tutta la settimana. `js/sito.js` applica la stessa regola nel browser |
